...
public class DefaultCodegen implements CodegenConfig {
			...
			if (mt.getEncoding() != null) {
				...
				for (Entry<String, Encoding> encodingEntry : encMap.entrySet()) {
					...
					CodegenEncoding ce = new CodegenEncoding(enc.getContentType(), headers, enc.getStyle().toString(),
							enc.getExplode().booleanValue(), enc.getAllowReserved().booleanValue());
					String propName = encodingEntry.getKey();
					ceMap.put(propName, ce);
				}
			}
			String contentType = contentEntry.getKey();
			CodegenProperty schemaProp = fromProperty(toMediaTypeSchemaName(contentType, mediaTypeSchemaSuffix),
					mt.getSchema());
	...
	private CodegenComposedSchemas getComposedSchemas(Schema schema) {
		...
		ComposedSchema cs = (ComposedSchema) schema;
		return new CodegenComposedSchemas(getComposedProperties(cs.getAllOf(), "allOf"),
				getComposedProperties(cs.getOneOf(), "oneOf"), getComposedProperties(cs.getAnyOf(), "anyOf"));
	}
